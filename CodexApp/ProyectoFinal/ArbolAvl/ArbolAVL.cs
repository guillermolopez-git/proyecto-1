...
namespace WindowsFormsApp1.Arbol_Avl
{
     class ArbolAVL
    {
        ...
        // metodo para buscar 
        public NodoAvl buscar(Object valor)
        {
            Comparador dato;
            Logical h = new Logical(false); // intercambia un valor booleano
            dato = (Comparador)valor;
            return buscarNodo(raizArbol(), dato, h);
        }

        private NodoAvl buscarNodo(NodoAvl raiz, Comparador dt, Logical h)
        {
            if (raiz == null)
            {
                return null;
            ...
            {
                Comparador nodoValor = (Comparador)raiz.valorNodo();

                if (dt.igualQue(nodoValor))
                {
                    return raiz;
                }
                else if (dt.menorQue(nodoValor))
                {
                    return buscarNodo((NodoAvl)raiz.subarbolIzdo(), dt, h);
                }
                else
                {
                    return buscarNodo((NodoAvl)raiz.subarbolDcho(), dt, h);
                }
            }
        }
    }
